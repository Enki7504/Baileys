...
import makeWASocket, { AnyMessageContent, delay, DisconnectReason, fetchLatestBaileysVersion, makeCacheableSignalKeyStore, makeInMemoryStore, PHONENUMBER_MCC, proto, useMultiFileAuthState, WAMessageContent, WAMessageKey } from '@whiskeysockets/baileys'
...
//ESTA ES LA VERSION FINAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAL
...
connection.connect((error) => {
    ...
    connection.query(sqlUsuarios, (errorUsuarios, resultsUsuarios) => {
        if (errorUsuarios) {
        ...
        connection.query(sqlMensajes, (errorMensajes, resultsMensajes) => {
            if (errorMensajes) {
            ...
            if (resultsMensajes.length > 0) {
                ...
                img = row.img ? row.img.replace(/^"|"$/g, '') : '';  // Quita las comillas al principio y al final 
            }
                        ...
                        async (events) => {
                            if (events['connection.update']) {
                                ...
                                if (connection === 'close') {
                                    ...
                                    if (img) {
                                        for (const contacto of usuarios) {
                                            let random = Math.floor(Math.random() * mensajes.length);
                                            let mensaje = mensajes[random];
                                            ...
                                            console.log("Mensaje mandado a " + contacto.nombre+": "+mensaje)
                                        }
                                    } else {
                                        for (const contacto of usuarios) {
                                            let random = Math.floor(Math.random() * mensajes.length);
                                            ...
                                            console.log(random)
                                            console.log("Cantidad de mensajes posibles")
